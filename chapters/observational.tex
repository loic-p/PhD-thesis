...
casting, which only need to be defined when the types are
compatible -- otherwise the cast will be stuck.
% 
\sideremark{Alternatively, we could reduce a cast between any two incompatible
...
\section{Comparing \SetoidCC and \MLTT}
\label{sec:comparison-MLTT}
...
\[
  \inferrule{\tytm{\Gamma}{A}{\Type_i}
			\\ \tytm{\Gamma}{t}{A}
			\\ \tytm{\Gamma}{B}{\Depfun{A}{\Fun{\Obseq[A]{t}{x}}{\Type_j}}}
...
the rules for the dependent products, dependent sums, universes and integers

and we can replace the Martin-Löf identity type with the observational 
equality, which provides the additional comfort of function extensionality and
proposition extensionality.

There is however a small discrepancy with the computation rule of the \( J \) 
eliminator. In \MLTT, applying \( J \) to a proof of reflexivity reduces
to the corresponding argument:
\[
	\J{A}{t}{P}{u}{t}{\refl{t}{t}} \quad \Longrightarrow \quad u
\]
but in \SetoidCC with our replacement for \( J \), this rule is weakened
and only holds up to a propositional equality that we can derive from 
\( \castreflName \).
% 
This is not a huge deal, as weakening computation rules in presence of UIP 
and funext does not change the expressive power of the theory \sidecite{weaktt},
but it is an obstacle to a direct translation from \MLTT to \SetoidCC.
% 
In this thesis, we will discuss two ways around this obstacle: either we change 
rule~\nameref{infrule:cast-refl} to be a definitional equality (see \shepherd{TODO}),
or we translate the Martin-Löf identity type to the type \( \metaop{Id} \) of 
\cref{sec:extensions}.
...
% 
\paragraph*{Box Types}
Box types embed the propositions into the proof-relevant world.
...
% 
As we explained in \cref{sec:comparison-MLTT}
