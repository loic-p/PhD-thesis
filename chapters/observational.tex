...
\begin{center}
  \begin{tabular}{lr}
	...
	\( \redannotated{\Gamma}{t}{u}{A}{\Univ} \) & \qquad {the term \( t \) reduces to \( u \) at type \( A \) in context \( \Gamma \)}
  \end{tabular}
\end{center}
...
\end{mathpar}
% 
\sideremark{Remark that rule~\nameref{infrule:nat-elim} applies to both 
...
In \SetoidCC however, every type is already a setoid (with the observational 
