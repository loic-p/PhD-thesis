...
theorem~\sidecite{BlakersMassey}
and Serre's spectral sequence \sidecite{vanDoorn18}. 
...
%
\sideremark{When we input this definition to \CubicalAgda, 
...
The \func{Glue} primitive takes a base type \var{B} and a partial family of 
types \var{A} that are equivalent to \var{B}, and produces type by ``gluing'' 
\var{A} onto \var{B}. The resulting type is definitionally equal to \var{A}
when it is defined.
%
...
\begin{figure}[!h]
  ...
  \end{tikzpicture}
  \caption{Graphical representation of \func{Glue}~\var{B}~\{\var{r}~=~\var{i}~\func{∨}~\func{∼{}}~\var{i}\}~\var{A}}
\end{figure}
...
\begin{marginfigure}
  ...
  \end{tikzpicture}
  \caption{Graphical represenation of the \func{helix} as a dependent type on \func{S¹}}
\end{marginfigure}
% 
...
\con{loop} case of \func{decode} is a not too difficult to define cubically,
