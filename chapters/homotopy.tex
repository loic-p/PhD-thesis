...
type theory have HoTT libraries: \texttt{HoTT-Agda} \sidecite{hottagda}, \texttt{Coq-HoTT}
\sidecite{hottcoq}, \texttt{Lean-HoTT} \sidecite{hottlean}, \texttt{UniMath}
\sidecite{UniMath}. Many impressive results can be found in these
...
theorem~\sidecite{BlakersMassey} % , the Gysin sequence~\sidecite{Brunerie16}
% \anders{I don't think this has been mechanized?}
and Serre's spectral sequence \sidecite{vanDoorn18}. However, despite
...
\sidecite{CCHM,AngiuliHouHarper18,ABCFHL}. This is a family of type
...
with less structure~\sidecite{AngiuliHouHarper18,ABCFHL}. All of the
...
cubical extension \sidecite{cubicalagda} of the \Agda{} proof assistant
\sidecite{Agda} that is based on the De Morgan variation of cubical type
...
The paper ends with some discussions of future work and a comparison
...
heavily inspired by the one of \sidecitet{CCHM}, more precisely on the
variation outlined by \sidecitet{CHM} that is well-suited for HITs. The
...
curious readers to the paper of \sidecitet{cubicalagda} for a
...
type \func{I} with endpoints \con{i0} and \con{i1}. This plays the
role of the real interval $[0,1] \subset \mathbb{R}$ in homotopy
theory, however in \texttt{Cubical Agda} this is a purely formal
object. A variable \var{i} : \func{I} represents a point varying
continuously between the two endpoints. The interval is equipped with
three basic operations: \emph{minimum} (\func{\_∧\_} : \func{I} $\to$
\func{I} $\to$ \func{I}), \emph{maximum} (\func{\_∨\_} : \func{I}
$\to$ \func{I} $\to$ \func{I}) and \emph{reversal} (\func{∼{}\_} :
\func{I} $\to$ \func{I}). These operations form a \emph{De Morgan
...
of types (\func{Set} \var{ℓ}) represents a line between two types. By
...
the quantification of the universe level \var{ℓ} for readability):
%
...
The \func{PathP} types should be thought of as representing
heterogeneous equalities since the two endpoints are in different
types; this is similar to the dependent paths in
HoTT~\sidecite[][Sect. 6.2]{HottBook13}. We define the type of non-dependent
...
argument \var{A} (first \var{A}) to a variable $A$ (second $A$) that
...
We can also use the basic operations on \func{I} to construct various
useful operations, for example the reversal of a path is defined
using \func{∼{}\_} and represents the fact that \func{≡} is symmetric.
%
\ExecuteMetaData[chapters/literate-agda/Code.tex]{sym}

\subsection{Transport and Composition}

One of the key operations with type theoretic equality is
...
using another primitive called \func{transp}. However, for the
examples in this paper the \func{transport} function suffices.
%
\ExecuteMetaData[chapters/literate-agda/Code.tex]{transport}
%
% The more general \func{transp} operation lets us specify where the
% transport is the identity function. In particular, when the second
% argument to \func{transp} is \con{i1} it will reduce to \var{a}, which
% let us prove that \func{transp}\;\var{A}\;\var{r}\;\var{a} is always
% path equal to \var{a}.
% \loic{This remark seems too technical. Hopefully we can get rid of
%   transp in this paper...}
% \anders{Yeah, I agree. Hopefully we can drop the actual definition of
%   transport and pretend like it's the only operation.}
% However, for the examples in this paper the \func{transport} function
% will be sufficient.
% \anders{Double check that this is true.}

The substitution principle is an instance of \func{transport}.
%
\ExecuteMetaData[chapters/literate-agda/Code.tex]{subst}
%
This function invokes \func{transport} with a proof that the family
...
Martin-Löf's inductively defined identity types~\sidecite{MartinLoef75}.
%
...
the computation rule when applied to \func{refl}. Nevertheless, we can
...
equal to a numeral as proved by~\sidecitet{Huber16}.

\texttt{Cubical Agda} also has a primitive operation for composing
...
\func{Partial}~\var{r}~\var{A}. The idea is that
\func{Partial}~\var{r}~\var{A} is the type of cubes in \var{A} that
are only defined when (\var{r} = \con{i1}) holds. For example,
\func{Partial}~(\var{i}~\func{∨}~\func{∼{}}~\var{i})~\var{A} is a type
that is only defined when \var{i} is \con{i1} or \con{i0} (this means
...
\con{true} is not equal to \con{false}.

\texttt{Cubical Agda} also has cubical subtypes as in~\sidecitet{CCHM};
given \var{A} : \func{Set}~\var{ℓ}, \var{r} : \func{I} and
\var{u}~:~\func{Partial}~\var{r}~\var{A} we can form the type
\var{A}~\func{[}~\var{r}~\func{↦}~\var{u}~\func{]}.  A term \var{v} of
this type is a term of type \var{A} that is definitionally equal to
\var{u} when (\var{r} = \con{i1}) is satisfied. Any term \var{u} :
\var{A} can be seen as a term of type
\var{A}~\func{[}~\var{r}~\func{↦}~\var{u}~\func{]} that agrees with
itself when (\var{r} = \con{i1}):
%
...
When calling \func{hcomp} \{r = \var{r}\} \var{u} \var{u₀},
\texttt{Cubical Agda} makes sure that \var{u₀} agrees with \var{u}
\con{i0} on \var{r}; this is specified in the type of \var{u₀}. The
idea is that \var{u₀} is the base and \var{u} specifies the sides of
an open box where the side opposite of \var{u₀} is missing. The
\func{hcomp} operation then gives us the missing side.
...
% When \var{i} is \con{i0} this is \func{outS}~\var{u₀} and when \var{i}
% is \con{i1} it is \func{hcomp} ($\lambda$ \var{j} $\to$ $\lambda$ \{
% (\var{r} = \con{i1}) $\to$ \var{u} \var{j} \func{1=1} \} \var{u₀} as
...
% between \func{outS}~\var{u₀} and \func{hcomp} \var{u} \var{u₀} which
...
can define the circle HIT using an \texttt{Agda} \keyw{data} declaration.
...
$\cdot$ \con{loop} as in the above definition of \func{double}. In
general, for HITs
...
and \con{loop}. When typechecking definitions like the one for
\func{double}, \texttt{Cubical Agda} checks that the boundary of all
cases are correct, in particular that the defining equation
\[
  \func{double}~(\con{loop}~\var{i}) = (\con{loop} \cdot \con{loop})~\var{i}
\]
agrees with \( \func{double}\ \con{base} \) when \var{i} is \iz{} or
\io{}.
...
theory represents a higher dimensional cube, \func{Glue} types let us
...
\var{B} is a singleton type. We write \var{A} \func{≃} \var{B} if
...
The \func{Glue} types take a partial family of types \var{A} that are
equivalent to the base type \var{B}. These types are then ``glued''
onto \var{B} and the equivalence data gets packaged up into a new
type.
%
\ExecuteMetaData[chapters/literate-agda/Glue.tex]{Glue}
%
% When \var{r} is \con{i1} the type \func{Glue}\;\var{B}\;\var{Ae}
% reduces to \var{Ae}\;\func{1=1}\;\proj{fst}.

These types are introduced using the \func{glue} constructor and
eliminated using the \func{unglue} operation. Examples of this will be
discussed in the proof of \cref{thm:hopf}. Using \func{Glue} types we
...
Pictorially we can describe \func{ua} \var{e} as the dashed top line
in:
%
\begin{mathpar}
  \begin{tikzpicture}[xscale=2.5,yscale=2]
    \node (A01) at (0,1) {\var{A}};
    \node (A11) at (1,1) {\var{B}};
    \node (A00) at (0,0) {\var{B}};
    \node (A10) at (1,0) {\var{B}};
    \path[->,font=\scriptsize,>=angle 90]
      (A01) edge node[left]{\var{e}} node[right]{\reflectbox{\rotatebox[origin=c]{90}{$\simeq$}}} (A00)
      (A11) edge node[left]{\reflectbox{\rotatebox[origin=c]{90}{$\simeq$}}} node[right]{\func{idEquiv}~\var{B}} (A10)
      (A00) edge node[below]{\var{B}} (A10);
    \path[->,font=\scriptsize,>=angle 90,dashed]
      (A01) edge node[above]{$ $} (A11);
  \end{tikzpicture}
\end{mathpar}
...
however, for all of the examples in this paper the \func{ua} function
suffices.
...
\texttt{Cubical Agda} was given by \sidecitet[][Section 2.4.1]{cubicalagda},
...
\func{c2t-t2c} and \func{t2c-c2t} surprisingly nontrivial in HoTT.
...
function). By applying \func{ua} we then get a nontrivial
...
\con{loop} case of \func{decode} is a bit longer to define and we omit
...
In this drawing $A$ is a circle, in which case the suspension is a
sphere. This generalizes well, and we can define the higher spheres as
...
write the higher constructor corresponding to \con{surfₙ} in \texttt{Cubical
...
Pushouts are often nested, and when dealing with them a fact known as
the \( 3 \times 3 \) lemma comes in very handy.
...
case (which has been temporarily called \func{filler}).
...
In order to fix this, we will use \func{hcomp} to glue the four
...
To do this we hence need to construct the squares $s1$--$s5$, in
this order.

To construct $s1$, we unfold \( \func{f□1}\, (\con{push}\, a\, j) \) on
the left side, obtaining a composition of three paths. But functions
commute with \func{hcomp}'s up to a homotopy, so \func{A□0-A○□} applied to
the composition is homotopic to the composition of the images of the
...
\end{tikzpicture}
\end{center}
Once rotated counter-clockwise, we can use this square for $s1$ and
the analogue for $s2$. After simplifying the applications of
\func{A□0-A○□}, we are left with the following boundary:
%
\begin{center}
\begin{tikzpicture}[xscale=1,yscale=1]
  \draw [thin, -latex'] (0.6,0.6) -- (1.4,0.6) ;
...
By combining these five squares using \func{hcomp} we obtain the
definition of \func{filler}, and thus of our first map.
...
characterization of \func{S³} as the join of two circles which will be
...
\func{S¹} \func{S¹} and \func{S³}. Evan Cavallo has found a more
...
is equal to \func{S³}. It is a very useful construction that allows
one to compute homotopical properties of \func{S³} from the properties
of \func{S²} and \func{S¹}.

%% \loic{Should we get rid of this and replace it with pushout stuff only?}
%% \anders{No, it is fine}
We begin by motivating the definition using some geometry while
keeping in mind that we are actually working synthetically. Since the
two hemispheres of \func{S²} are contractible, the fibration is
...
the equator is equal to \func{S¹}, it amounts to defining a map
...
%% \anders{what is $U(1)$?}
%% \loic{Standard notation for the complex unit circle, from differential geometry.
%%   We dont really need it though.}
\[
e^{2i\pi \theta} \cdot e^{2i\pi \phi} = e^{2i\pi (\theta + \phi)}
\]
%% \loic{Am I getting too far off-topic? I think it is important to
%%   insist on how visual this is, but on the other hand...}
%% \anders{I think this is good, maybe point out that this is the
%%   point-set topological (or ``real'') intuition behind what is going
%%   on but that we are working synthetically.}
This is defined in \texttt{Cubical Agda} as follows:
%
\ExecuteMetaData[chapters/literate-agda/Circle.tex]{rot}

The final case can be illustrated by the following diagram, where we
...
\( \func{rot}\ (\con{loop}\ j)\ (\con{loop}\ k) \) is analogous to
...
This results in a binary operation on \func{S¹}, that we often write
...
(which is equivalent to \func{S²} by \cref{lem:susps23}) as our base
space, using \func{Glue} to glue the fibers as discussed
above. However, we have to depart slightly from the topological
intuition as there is no actual equator in
\( \func{Susp}\ \func{S¹} \). As instead, we use \func{rot} to glue
the fibers around the \con{north} pole, and the identity to glue them
around the \con{south} pole:
%
\ExecuteMetaData[chapters/literate-agda/Hopf.tex]{hopf}

In fact, we could have directly defined the Hopf fibration for
\func{S²} by gluing on the identity equivalence on three of the sides
...
\begin{proof}
  ...
  the longitude in \func{S²}, the second parameterizing the fiber.
  When transporting along the meridian \( \con{merid}\ \var{x} \), the
  fiber above \con{north} will transform into the fiber above the
  equator according to \( \func{rot}\ \var{x} \),
  %% \anders{What is the inverse operations here? Is it the normal sym or
  %%   the one defined by cases?}
  and the one above \con{south} will be transported to the equator
  according to the identity. Conversely, transporting fibers from the
  equator to the \con{north} (resp. \con{south}) pole will transform
  ...
  %% \anders{I'm a bit confused by this remark; why do we talk about
  %%   natural equivalences as opposed to just isomorphisms?}
  %% \loic{is this better?}
  %% \anders{Yes!}
  ...
  %%\anders{Now we suddenly talk about natural isos... }
  ...
  Here, \( x \) is the point in the fiber above \con{north}, \( y \)
  is the point in the fiber above \con{south}, and
  ...
  i)$ which in turn is a \func{Glue} type we need to use the
  \func{glue} constructor to package it up into a well-typed term.
  ...
  possible through the \func{unglue} primitive in \texttt{Cubical Agda}. Given
  ...
  %% However, as far as the computational content of the Hopf fibration
  %% is concerned, only the (very simple!) definitions of \func{h2j} and
  %% \func{j2h} are relevant. \loic{Reviewer 3 disagrees. investigate.}
  %% \anders{what to say? we can't go into too much
  %%   detail, but we can at least mention that it uses connectedness of
  %%   S1 (right?) and ...?}
  %% \loic{I think this is more of a hack to avoid annoying path algebra,
  %%   and that it is possible to do it without using any sort of
  %%   abstract stuff...}
  %% \loic{Maybe mention that these two maps is pretty much all of the
  %%   relevant computational content.}
\end{proof}

This hence concludes our direct cubical proof that \func{Hopf} is
...
\texttt{Cubical Agda} called \cubicaltt{} \sidecite{Cubicaltt}.
...
theory: \texttt{HoTT-Agda} \sidecite{hottagda}, \texttt{Coq-HoTT} \sidecite{hottcoq}, \texttt{Lean-HoTT}
\sidecite{hottlean}.\footnote{We omit the \texttt{UniMath} library as it does not
...
\sidecitet{LicataBrunerie15}. In a cubical system like \texttt{Cubical
...
(\func{∼{}\_}) from \texttt{Cubical Agda} any internal result about homotopy
