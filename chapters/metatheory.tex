...
Thus, following \sidecitet{Abel:POPL2018}, we define the notion of 
...
(Rules \nameref{infrule:eq-form}, \nameref{infrule:cast}, and \nameref{infrule:pi-irr-form}).
...
\begin{lemma}
  The rules \nameref{infrule:pi-irr-form}, \nameref{infrule:pi-irr-intro} and \nameref{infrule:pi-irr-elim} preserve validity.
\end{lemma}

\begin{proof}
  The case of \nameref{infrule:pi-irr-form} is easy, as proving reducibility 
  of a proof-irrelevant \( \Pi \)-type only requires proving that the domain
  and codomain are well-typed, which we obtain from the validity hypotheses.

  The case of \nameref{infrule:pi-irr-intro} is also a one-liner, because
  proving reducibility of an inhabitant of a proof-irrelevant type is only
  a matter of showing that it is well-typed.
...
Another direct consequence of the fundamental lemma is that any closed
term of type $\Nat$ reduces to a whnf of type $\Nat$. Thus, to
conclude canonicity, we just need to know that there is no neutral
term of type $\Nat$ in an empty context.
%
Unfortunately, as the notion of reduction is absent for
terms of proof-irrelevant types, and in particular for the empty type,
this cannot be proven by induction on the syntax of the neutral terms.
%
However, assuming consistency of \SetoidCC (proved in \cref{sec:cons-seto-model}), or in other words that
there is no closed term of type $\Empty$, we can derive canonicity of
\SetoidCC \refAgda{Typed.Consequences.}{Canonicity}.
