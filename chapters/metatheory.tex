...
%
From their model, Gilbert \etal also obtain a proof of consistency for \MLTT 
...
the fundamental lemma directly on the rules we presented in \cref{ch:observational}, 
...
reducibility predicates, but we are not, so we will prove our theorem for more
...
Remark that some type that we defined in \cref{ch:observational} do not appear 
...
\begin{proof}
  The case of the formation rule \nameref{infrule:pi-irr-form} is easy, as 
  ...
  The case of the elimination rule \nameref{infrule:pi-irr-elim} is more 
  ...
  \sideremark{Even though this change of rules gives the impression that the
    rules we presented in \cref{ch:observational} are informal rules that
    omit important premises, it is not the case: in \cref{sec:remov-paran-prem},
    we will show that the simplified rules of \cref{ch:observational} are 
    in fact equivalent to the rules with all the required hypotheses.}
  % 
  \[
...
work of \sidecitet{meven_thesis} on bidirectional type-checking, who explain 
...
Therefore, the reader may wonder if the rules that we enunciated in 
