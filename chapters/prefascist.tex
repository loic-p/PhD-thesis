% \setchapterimage[6cm]{fasciation}
% \setchapterpreamble[u]{\begingroup \marginnote[-48mm]{{\begin{flushright} \textcolor{white}{A fasciated~daisy~flower\\Picture from ``le blog de Blanche''} \end{flushright}}} \endgroup \margintoc}
...
full subcategory of \func{Set}, in which case the morphisms are 
...
\begin{center}
    \sideremark{The horizontal equalities are instances of \field{F\_comp},
        and the two vertical equalities hold definitionally.}
...
\( A \) are convertible.
% 
We also add a strict truncation operator \func{∥\_∥ₛ} that reflects 
...
\func{F\_comp} definitionally, and a morphism is defined in a similar way.
...
and since \func{F₁} satisfies the functoriality equations up to a definitional equality, 
...
access to a sort \func{Prop} of strict propositions.
...
\begin{proof}
A non-strict category consists of a type of objects \func{Obj}, a \emph{hset} of 
...
\sideremark{We do not risk any higher coherence issues by using
    propositional equality, since we assumed that the morphism 
    types of \( \cat{C} \) are set-truncated.}
