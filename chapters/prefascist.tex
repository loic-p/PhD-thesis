...
\sideremark{This constraint might look very strong, but in the next section
...
types owe their eyebrow-raising name to the French word for sheaf, 
\textit{faisceau}, and to the humor of Pierre-Marie Pédrot.}, an original
...
Moreover, we will extend \MLTT with a sort of strict propositions \func{SProp},
...
in \func{SProp} is proof-irrelevant, meaning that all terms with type 
\( A \) are convertible.
% 
We also add a strict truncation operator \func{∥\_∥ₛ} that embeds 
proof-relevant types in \func{SProp}.

Using \func{SProp}, we can have the category \( \Delta_+ \) of finite ordinals 
...
\subsection{Parametric Sections of Presheaves}

We start by noticing that if \( F \) is a presheaf and \( p \) an object of 
\( \cat{C} \), then an element of \( F(p) \) should be considered along with
if family of
\defnote{restrictions}{If \( \alpha \) is a morphism of \( \hom{q}{p} \), then 
\( F(\alpha) \) is called the restriction operation along \( \alpha \). This 
terminology comes from the time where presheaves were always defined on 
topological spaces.}
along the morphisms with codomain \( p \).
\[
\AgdaSymbol{∀}\ q\ (α : \func{Hom}\ q\ p)\ \AgdaSymbol{→}\ F\ q
\]
